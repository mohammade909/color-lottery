...
//   // Admin endpoints (should be protected by admin role guard)
//   @Post('admin/force-end-all')
//   @ApiOperation({ summary: 'Force end all active games (Admin only)' })
//   @ApiResponse({ status: 200, description: 'All games ended successfully' })
//   // @UseGuards(AdminGuard) // Uncomment and implement this guard
//   async forceEndAllGames(): Promise<{ message: string }> {
//     try {
//       await this.colorGameService.forceEndAllGames();
//       return { message: 'All games ended successfully' };
//     } catch (error) {
//       throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
//     }
//   }
