...
export class WebSocketController {
  ...
  @ApiResponse({ status: 200, description: 'Return all games with details', type: StandardResponseDto })
  @ApiQuery({ name: 'active', required: false, type: Boolean, description: 'Filter by active status' })
  @ApiQuery({ name: 'duration', required: false, type: String, description: 'Filter by game duration' })
  ...
  @ApiResponse({ status: 200, description: 'Return all active games', type: StandardResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Return game details', type: StandardResponseDto })
  async getGameById(@Param('gameId') gameId: string): Promise<StandardResponseDto<any>> {
    try {
  ...
  @ApiResponse({ status: 200, description: 'Return game bets', type: BetStatisticsResponseDto })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Limit number of results' })
  async getGameBets(
  ...
  @ApiResponse({ status: 200, description: 'Return comprehensive game statistics', type: GameStatisticsResponseDto })
  @ApiQuery({ name: 'days', required: false, type: Number, description: 'Number of days to include in statistics' })
  async getGameStatistics(@Query('days') days?: number): Promise<GameStatisticsResponseDto> {
    try {
  ...
  @ApiResponse({ status: 200, description: 'Return statistics for specific duration', type: StandardResponseDto })
  async getStatisticsByDuration(@Param('duration') duration: string): Promise<StandardResponseDto<DurationStatisticsDto>> {
    try {
      const statistics = await this.gameService.getStatisticsByDuration(duration);
  ...
  @ApiResponse({ status: 200, description: 'Return user bets', type: UserStatisticsResponseDto })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Limit number of results' })
  async getUserBets(
  ...
  @ApiResponse({ status: 200, description: 'Return recent game results', type: StandardResponseDto })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Limit number of results' })
  async getRecentResults(@Query('limit') limit?: number): Promise<StandardResponseDto<any[]>> {
    try {
  ...
  // @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Get recent game results' })
  @ApiResponse({ status: 200, description: 'Return Transactions', type: StandardResponseDto })
  // @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Limit number of results' })
  ...
  @ApiResponse({ status: 200, description: 'All games force ended', type: StandardResponseDto })
