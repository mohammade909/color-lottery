...
export class UsersService {
  ...
  async update(id: string, updateUserDto: UpdateUserDto): Promise<Omit<User, 'password'>> {
    ...
    if (updateUserDto.password) {
      const salt = await bcrypt.genSalt();
      updatedUserData.password = await bcrypt.hash(updateUserDto.password, salt);
    }
 ...
 async withdrawal(id: string, amount: number, description?: string): Promise<Omit<User, 'password'>> {
  const user = await this.findById(id);
  if (!user) {
    throw new NotFoundException(`User with ID ${id} not found`);
  }
  ...
  if (user.wallet < amount) {
    throw new BadRequestException(`Insufficient balance for withdrawal of ${amount}`);
  }
  ...
  const updatedUser = await this.findById(id);
  if (!updatedUser) {
    throw new NotFoundException(`User with ID ${id} not found after withdrawal`);
  }
...
async deposit(id: string, amount: number, description?: string): Promise<Omit<User, 'password'>> {
  const user = await this.findById(id);
  if (!user) {
    throw new NotFoundException(`User with ID ${id} not found`);
  }
